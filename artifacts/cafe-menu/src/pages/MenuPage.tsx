  ...
  return (
    <div dir={isRtl ? 'rtl' : 'ltr'}
      ...
      <div className="max-w-lg mx-auto px-3 pt-3">
        <div className="relative">
          <svg className="absolute top-1/2 -translate-y-1/2 w-4 h-4 pointer-events-none"
            style={{ [isRtl ? 'right' : 'left']: '12px', color: theme.text_color + '55' }}
          ...
          <input
            ...
            className="w-full rounded-2xl text-sm py-3 outline-none transition-all"
            style={{
              backgroundColor: theme.card_color,
              color: theme.text_color,
              border: `1.5px solid ${search ? theme.primary_color + '88' : theme.border_color}`,
              paddingInlineStart: '40px',
              paddingInlineEnd: search ? '40px' : '16px',
            }}
          />
          {search && (
            <button onClick={() => setSearch('')}
              className="absolute top-1/2 -translate-y-1/2 w-6 h-6 rounded-full flex items-center justify-center transition-all"
              style={{ [isRtl ? 'left' : 'right']: '10px', backgroundColor: theme.border_color, color: theme.text_color + '99' }}>
      ...
      <main className="max-w-lg mx-auto px-3 py-4 pb-28">
        {!loading && filteredItems.length > 0 && !q && (
      ...
      {/* ===== FLOATING BUTTONS ===== */}
      <div className="fixed bottom-6 z-40 flex flex-col gap-3" style={{ [isRtl ? 'left' : 'right']: '16px' }}>
        {/* Scroll to top */}
        <AnimatePresence>
          {showScrollTop && (
            <motion.button
              ...
              style={{ backgroundColor: theme.card_color, border: `1.5px solid ${theme.border_color}`, color: theme.text_color + 'BB' }}>
        ...
        <motion.a
          ...
          style={{ backgroundColor: '#25D366', boxShadow: '0 8px 24px rgba(37,211,102,0.45)' }}>
